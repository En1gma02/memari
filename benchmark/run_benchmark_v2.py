"""
run_benchmark_v2.py - RAG Benchmark Orchestrator V2

Uses the enhanced RAG engine with:
- Query decomposition and expansion
- MMR diversity re-ranking
- Self-RAG critique loop

Usage: python run_benchmark_v2.py
"""
...
from rag_benchmark_v2 import get_benchmark_engine_v2
...
# V2: Disable Self-RAG for speed (can enable for more thorough evaluation)
USE_SELF_RAG = False
...
def run_benchmark():
    print("=" * 70)
    print("  RAG Benchmark Suite V2")
    print("  Improvements: Query Expansion, MMR Diversity, Self-RAG")
    ...
    print(f"\n[2/4] Initializing RAG engine V2...")
    engine = get_benchmark_engine_v2()
    
    print(f"\n[3/4] Running benchmark (Self-RAG: {USE_SELF_RAG})...")
    results = []
    ...
    for query_data in tqdm(queries, desc="Benchmarking V2"):
        ...
        # V2 hybrid search with enhancements
        retrieval_result = engine.hybrid_search(query, top_k=TOP_K)
        
        # Generate answer (with optional Self-RAG)
        generation_result = engine.generate_answer(query, retrieval_result.chunks, use_self_rag=USE_SELF_RAG)
        ...
        result = {
            ...
            "retrieved_chunk_ids": str(retrieval_result.chunk_ids),
    ...
    summary = {
        ...
        "version": "v2",
        "improvements": ["query_expansion", "query_decomposition", "mmr_diversity", f"self_rag={USE_SELF_RAG}"],
        "num_queries": n,
        "top_k": TOP_K,
        "retrieval_metrics": {
